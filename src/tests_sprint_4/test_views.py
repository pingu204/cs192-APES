...
class AddCourseToSchedViewTest(TestCase):
    def setUp(self):
        ...
        self.saved_schedule = SavedSchedule.objects.create(
            student_id=self.user.id, sched_id=1, schedule_name="Test Schedule"
        )
    ...
    def test_add_course_to_schedule_success(self):
        request = self.factory.post(
            reverse(
                ...
                kwargs={"sched_id": self.saved_schedule.sched_id},
            ),
        ...
        response = add_course_to_sched_view(
            request, sched_id=self.saved_schedule.sched_id
        )
    ...
    def test_remove_class_from_saved_schedule(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertFalse(
            SavedCourse.objects.filter(course_code="CS 194").exists()
        )  # Ensure the course is removed
...
class RedrawCourseToSchedViewTest(TestCase):
    ...
    def test_redraw_course_success(self):
        ...
        request = self.factory.post(
            reverse(
                ...
                kwargs={
                    "sched_id": self.saved_schedule.sched_id,
                    "course_code": "CS 192",
                },
            ),
        ...
        with patch("courses.views.get_all_sections", return_value=[new_course_data]):
            response = redraw_course_to_sched(
                request, sched_id=self.saved_schedule.sched_id, course_code="CS 192"
            )

        self.assertEqual(response.status_code, 302)  # Redirect to view_saved_sched_view
        self.assertTrue(
            SavedCourse.objects.filter(
                course_code="CS 192",
                course_details__section_name={"lec": "TBC1", "lab": "TBC1/HQR1"},
            ).exists()
        )  # Ensure the new course is added
        self.assertFalse(
            SavedCourse.objects.filter(
                course_code="CS 192",
                course_details__section_name={"lec": "TDE1", "lab": "TDE1/HUV1"},
            ).exists()
        )
    ...
    def test_redraw_course_not_found(self):
        ...
        response = redraw_course_to_sched(
            request, sched_id=self.saved_schedule.sched_id, course_code="CS 999"
        )
    ...
    def test_redraw_course_conflicting_sections(self):
        ...
        conflicting_course_data = {
            ...
            "timeslots": {
                "lab": [0, 180],
                "lec": [60, 180],
            },  # Conflicts with existing course
            "class_days": {"lab": "H", "lec": "T"},
            "offering_unit": "DCS",
            "instructor_name": {
                "lab": "FIGUEROA, LIGAYA LEAH",
        ...
        with patch(
            "courses.views.get_all_sections", return_value=[conflicting_course_data]
        ):
            response = redraw_course_to_sched(
                request, sched_id=self.saved_schedule.sched_id, course_code="CS 192"
            )
