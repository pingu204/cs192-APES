...
from apes.utils import (
    redirect_authenticated_users,
    guest_or_authenticated,
    get_course_details_from_csv,
)
...
def homepage_view(request, *args, **kwargs):
    ...
    if request.method == "POST" and "removed_course" in request.POST:
        ...
        if request.user.is_authenticated:
            ...
            DesiredCourse.objects.filter(
                student_id=request.user.id, course_code=removed_course_code
            ).delete()
        else:
            print(f"Guest removing {removed_course_code}")
            request.session["dcp"] = [
                course
                for course in request.session.get("dcp", [])
                if course["course_code"] != removed_course_code
            ]

        request.session["dcp_sections"] = [
            section_lst
            for section_lst in request.session.get("dcp_sections", [])
            if section_lst[0]["course_code"] != removed_course_code
        ]

        print(
            f"Session's `dcp_sections` now has {len(request.session['dcp_sections'])} sections."
        )
    ...
    if request.method == "POST" and "reset_preferences" in request.POST:
        if (
            "preferences" not in request.session
            or not request.session["preferences"]
            or all(x == None for x in request.session["preferences"].values())
        ):
            pass
        else:
            request.session["preferences"] = {
                k: None for k in request.session["preferences"]
            }
            request.session["raw_preferences"] = {
                k: None for k in request.session["raw_preferences"]
            }
    ...
    print(
        "\n".join(
            [
                f"+ {code} [{len(dcp_sections[i])} sections]"
                for i, code in enumerate(dcp_codes)
            ]
        )
    )
...
def guest_login_view(request, *args, **kwargs):
    ...
    request.session["is_guest"] = (
        True  # might be useful if we choose to separate guest from user dashboards// if guest -> save session, else flush after
    )
