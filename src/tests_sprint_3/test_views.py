...
from courses.views import view_sched_view
...
class GeneratePermutationViewTest(TestCase):
        ...
        self.dcp_sections = [
            [
                ...
                },
                {
                    ...
                    'class_days': {'lec': 'T', 'lab': 'H'},
                    'offering_unit': 'DCS',
                    'instructor_name': {'lec': 'SOLAMO, MA. ROWENA', 'lab': 'SOLAMO, MA. ROWENA'},
    ...
    def test_generate_permutation_view(self):
        ...
        self.assertEqual(request.session['schedule_permutations'], expected_permutations)
...
class ViewSavedSchedViewTest(TestCase):
    ...
    def test_view_sched_view_save_schedule(self):
        ...
        response = self.client.post(reverse('view_sched_view', args=[2]), {'click_saved_sched': 'Save Schedule'})
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.url, reverse('homepage_view'))

        saved_schedule = SavedSchedule.objects.filter(student_id=self.user.id, sched_id=2).first()
