...
class AddClassTestCase(TestCase):
    def setUp(self):
        ...
        self.user = Student.objects.create_user(
            username="LaboratoryKit",
            password="NinjasInParis123",
        ...
        self.session["dcp"] = []
        self.session["course_sections"] = [
            {
                "course_code": "CS 20",
                "course_title": "Digital Electronics and Circuits",
                "units": 3.0,
                "timeslot": {"M": [60, 180]},
                "offeringunit": "DCS",
            },
    ...
    def test_add_duplicate_class(self):
        ...
        response = self.client.post(
            reverse("dcp_add_view"), {"course_code": self.course_code}
        )
...
class HomepageViewTest(TestCase):
    ...
    def test_remove_course_authenticated_user(self):
        self.client.login(username="LaboratoryKit", password="NinjasInParis123")
        response = self.client.post(
            reverse("clear_desired_courses"), {"removed_course": self.course_code}
        )
        self.desired_course.delete()
        self.assertEqual(response.status_code, 302)
        self.assertFalse(
