...
def modify_preferences_view(request):
    if request.method == "POST":  # and 'save_changes' in request.POST:
        ...
        if form.is_valid():
            ...
            earliest_time_raw = next(
                (
                    i
                    for i, (val, _) in enumerate(time_choices)
                    if val == earliest_time_val
                ),
                None,
            )
            latest_time_raw = next(
                (
                    i
                    for i, (val, _) in enumerate(time_choices)
                    if val == latest_time_val
                ),
                None,
            )
            ...
            earliest_time = (
                ((earliest_time_raw * 15) - 15) if earliest_time_raw else None
            )
            ...
            request.session["preferences"] = {
                ...
                "total_distance_per_day": total_distance_per_day
                if total_distance_per_day
                else None,
                "total_probability": total_probability if total_probability else None,
                "earliest_time": earliest_time
                if not (earliest_time is None or latest_time_val is None)
                else None,
                ...
                "earliest_time_display": earliest_time_val
                if earliest_time_val and latest_time_val
                else None,
                "latest_time_display": latest_time_val
                if earliest_time_val and latest_time_val
                else None,
                "min_break_display": f"{min_break // 60} hours"
                if (min_break % 60 == 0) and min_break
                else f"{min_break} minutes",
                "max_break_display": f"{max_break // 60} hours"
                if (max_break % 60 == 0) and max_break
                else f"{max_break} minutes",
                "min_break_sched_display": f"{min_break // 60}-hour"
                if (min_break % 60 == 0) and min_break
                else f"{min_break}-minute",
                "max_break_sched_display": f"{max_break // 60}-hour"
                if (max_break % 60 == 0) and max_break
                else f"{max_break}-minute   ",
            }
    ...
    context = {"form": form, "session": request.session}
