...
def generate_time_choices():
    ...
    end_time = datetime.strptime("10:00 PM", "%I:%M %p")  # Last selectable time (10:00 PM)
...
class PreferencesForm(forms.Form):
    ...
    def clean(self):
        ...
        if earliest_time != "" and latest_time != "":
            earliest_index = next((index for index, value in enumerate(time_choices) if value[0] == earliest_time), None)
            latest_index = next((index for index, value in enumerate(time_choices) if value[0] == latest_time), None)

            if earliest_index is not None and latest_index is not None and earliest_index >= latest_index:
                self.add_error('latest_time', 'Earliest time must be earlier than latest time.')
