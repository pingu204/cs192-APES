...
def find_class(
    classes: list[Course], day: str, t: int
) -> tuple[str, ClassStatus, int, int] | None:
    # for c in classes:
    ...
    for i, c in enumerate(classes):
        ...
        for classType, (start, end) in c.timeslots.items():
            ...
            if start == t:
                ...
                location = c.venue.get(
                    classType, "Unknown Location"
                )  # edited since KeyError raised (e.g., CS 194)

                return (
                    f"<b>{course_code}</b><br>{timeslot}<br>{location}",
                    ClassStatus.STARTS_AT,
                    end - start,
                    i,
                )
...
def generate_timetable(classes: list[Course], glow_idx: int = -1):
    ...
    for i in range(17 * 4):  # 17 hours, 4 offset per hour
        ...
        main_row_str += (
            f"<th>{get_time(offset=i * 15)}</th>" if is_bound else "<th></th>"
        )
        export_row_str += (
            f"<th>{get_time(offset=i * 15, extended=False)}</th>"
            if is_bound
            else "<th></th>"
        )
        ...
        for day in ["M", "T", "W", "H", "F", "S"]:
            ...
            if current_class:
                ...
                main_row_str += "<td></td>"
                export_row_str += "<td></td>"

        # Append row string to the output container
        main_output.append(
            f"<tr>{main_row_str}</tr>"
            if not is_bound
            else f'<tr class="bound">{main_row_str}</tr>'
        )
        export_output.append(
            f"<tr>{export_row_str}</tr>"
            if not is_bound
            else f'<tr class="bound">{export_row_str}</tr>'
        )

    # Collate row strings
    main_output = [main_days_header] + (main_output + [main_last_header])[
        (start // 4) * 4 : (end // 4 + 1) * 4 + 1
    ]
    export_output = [export_days_header] + (export_output + [export_last_header])[
        (start // 4) * 4 : (end // 4 + 1) * 4 + 1
    ]
