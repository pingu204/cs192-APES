...
def dcp_add_view(request):
    ...
    if request.method == "POST":
        ...
        if course_code in dcp_codes:
            ...
            total_units + (get_course_details_from_csv(course_code))[0]["units"]
        ) > 30:
...
def generate_permutation_view(request):
    ...
    if student_id:
        saved_sched_ids = set(
            SavedSchedule.objects.filter(student_id=student_id).values_list(
                "sched_id", flat=True
            )
        )
    ...
    for day in ["M", "T", "W", "H", "F", "S"]:
        ...
        for c in classes:
            for classType, d in c.class_days.items():
                ...
                if day in d:
                    day_classes.append(
                        (c, c.location[classType], c.timeslots[classType])
                    )
                    # print('day:', day, 'course_code:', c.course_code, 'location:', c.location[classType])

        # print('day: ', day, day_classes)
        day_classes = sorted(
            day_classes, key=lambda x: x[2]
        )  # sort by timeslot of the class
...
def view_sched_view(request, sched_id: int):
    ...
    selected_schedule = next(
        (sched for sched in schedule_permutations if sched["sched_id"] == sched_id),
        None,
    )
    ...
    if request.method == "POST" and "click_saved_sched" in request.POST:
        ...
        SavedSchedule.objects.filter(student_id=student_id, sched_id=sched_id).delete()
...
def view_saved_sched_view(request, sched_id: int):
    ...
    if request.method == "POST" and "click_unsaved_sched" in request.POST:
        ...
        saved_schedule = SavedSchedule.objects.filter(
            student_id=student_id, sched_id=sched_id
        ).first()
    ...
    saved_schedule = get_object_or_404(
        SavedSchedule, student_id=student_id, sched_id=sched_id
    )
    ...
    if request.method == "POST" and "class_to_remove" in request.POST:
        ...
        course_data = eval(
            f"dict({course_data_str.replace('Course(', '').rstrip(')')})"
        )

        # Get the course to remove
        course_to_remove = saved_schedule.courses.filter(
            course_code=course_data["course_code"]
        ).first()  # based on course code instead of course_details

        # Check if course exists before removing
        if course_to_remove:
            if saved_schedule.courses.count() == 1:  # Only 1 course left
                saved_schedule.courses.remove(
                    course_to_remove
                ...
                if saved_schedule.courses.count() == 0:  # NOW IT WORKS!!!!!!!!!!!!
                    ...
                    messages.success(
                        request, f"Schedule {sched_id + 1} unsaved successfully!"
                    )
                ...
                if course_to_remove:  # Check if not None before deleting
                    course_to_remove.delete()  # Delete the course instance
                    messages.success(
                        request, "Class has been removed from the schedule."
                    )
        else:
            # Placeholder for when the course is not found in the schedule
            messages.error(
                request,
                f"Course {course_data['course_code']} not found in the schedule.",
            )
                ...
                if day in section_days:
                    ...
                    if timeslot:  # Check only if the timeslot exists and isn't empty
                        ...
                        for occupied_start, occupied_end in occupied_slots:
                            if not (
                                end_time <= occupied_start or start_time >= occupied_end
                            ):
...
def add_course_to_sched_view(request, sched_id: int):
    ...
    saved_courses = saved_schedule.courses.all()
    classes = [
        convert_timeslots_to_tuples(
            course.course_details
        )  # Unpack the dictionary properly
        for course in saved_courses
    ]
        ...
        if form.is_valid():
            ...
            for course in course_sections:
                ...
                flat_classes = [
                    ...
                    for course in (sublist if isinstance(sublist, list) else [sublist])
                ]
                flat_classes.append(course)  # add the new course to the classes
                if not has_conflict(
                    flat_classes
                ):  # newly created function on top of here
                    # print("Course OK: ", flat_classes)
                    search_results.append(
                        Course(**course)
                    )  # add the new course to the search results
    ...
    classes = [
        Course(
            **convert_timeslots_to_tuples(course.course_details)
        )  # Unpack the dictionary properly
        for course in saved_courses
    ]
    # Generate schedule tables
...
def redraw_course_to_sched(request, sched_id: int, course_code: str):
    ...
    if request.method == "POST" and "course_data" in request.POST:
        ...
        existing_course = saved_schedule.courses.filter(
            course_code=course_data["course_code"]
        ).first()
    ...
    course_details = next(
        (course for course in classes if course["course_code"] == course_code), None
    )
    ...
    for c in course_sections:
        ...
        if not has_conflict(temp):
            schedule_permutations.append(
                temp[:]
            )  # [:] so the pop wont affect the schedule_permutations
        temp.pop()

    if not schedule_permutations:
        messages.error(
            request,
            "Schedule containing the specified course with different sections not found.",
        )
        print("GO BACK")
        return redirect("view_saved_sched_view", sched_id=sched_id)
    ...
    for courses in schedule_permutations:
        ...
        changed = [Course(**course) for course in courses]
...
def filter_course_sections(course_sections, schedule_permutations):
    ...
    for schedule in schedule_permutations:
        for course in schedule:
            ...
            if "lab" in course["section_name"]:
                valid_courses.add(
                    (course["course_code"], course["section_name"]["lab"])
                )
    ...
    for course in course_sections:
        ...
        if (
            course["course_code"],
            course["section_name"].get("lec", ""),
        ) in valid_courses or (
            "lab" in course["section_name"]
            and (course["course_code"], course["section_name"].get("lab", ""))
            in valid_courses
        ):
