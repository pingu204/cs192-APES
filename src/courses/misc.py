    ...
    for day in "MTWHFS":
        ...
        if day in "".join(list(course["timeslots"].keys())):
            ...
            dcp_with_classes = list(
                filter(
                    lambda x: day in "".join(list(x["timeslots"].keys())), dcp_courses
                )
            )

            # Get range of intervals for the courses in DCP
            dcp_slot_ranges = [
                set(get_ranges(*get_start_and_end(timeslot=c["timeslots"], day=day)))
                for c in dcp_with_classes
            ]

            # Get range of intervals for the course to be added in DCP
            course_range = set(
                get_ranges(*get_start_and_end(timeslot=course["timeslots"], day=day))
            )
            ...
            if True in [
                course_range.intersection(slot_range) != set()
                for slot_range in dcp_slot_ranges
            ]:
                return True
