...
class TestViewsNotGuest(TestCase):
    ...
    def test_login_POST(self): 
        ...
        Student.objects.create_user(username='testuser', password='Iunderstanditnow123', email='test@example.com')
...
class TestViewsGuest(TestCase):
    ...
    def setUp(self):
        self.client = Client()
        self.login_url = reverse('guest_login_view')
    ...
    def test_guest_login_POST(self): 
        ...
        self.assertTrue(self.client.session.get('is_guest', False))  # Check if 'is_guest' is set to True

    def test_logout_POST(self):
        # Log in as guest first
        self.client.post(self.login_url)
        self.assertTrue(self.client.session.get('is_guest', False))  # Ensure guest login

        # Log out
        logout_url = reverse('logout_view')
        response = self.client.post(logout_url)
        self.assertEqual(response.status_code, 302)  # Assuming a redirect to landing view
        self.assertRedirects(response, reverse('landing_view'))
        self.assertFalse(self.client.session.get('is_guest', False))  # Ensure 'is_guest' is removed